...
class LoRATrainingRecipe(BaseTrainingRecipe):
    def __init__(self, training_arguments):
        ...
        self.lora_skip_module = ['connector', 'vision_tower', 'edge_fuser', 'language_model']
        
        
    def training_model_converse(self, model):
        # Stage-2 follows the paper: the visual encoder, connector and
        # pretrained semantic-transition bridge stay frozen; LoRA is
        # inserted only into the language model.  Freezing edge_fuser is
        # also crucial for memory because otherwise every pairwise bridge
        # state keeps an autograd graph during instruction tuning.
        if hasattr(model, 'edge_fuser'):
            model.edge_fuser.requires_grad_(False)
    ...
    def save(self, model, trainer):
        ...
        # save the pretrained semantic-transition bridge as a non-LoRA module
        if hasattr(model, 'edge_fuser'):
            edge_fuser_state_dict = get_peft_state_non_lora_maybe_zero_3(
                model.edge_fuser.named_parameters(), False
            )
            if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
