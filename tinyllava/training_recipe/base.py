...
class BaseTrainingRecipe:
    ...
    def save(self, model, trainer):
        ...
        # save semantic-transition bridge learned in stage-1
        if hasattr(model, 'edge_fuser'):
            edge_fuser_state_dict = get_state_maybe_zero_3(model.edge_fuser.named_parameters(), [''], False)
            if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
                edge_fuser_output_dir = os.path.join(self.training_arguments.output_dir, 'edge_fuser')
                os.makedirs(edge_fuser_output_dir, exist_ok=True)
                torch.save(edge_fuser_state_dict, os.path.join(edge_fuser_output_dir, 'pytorch_model.bin'))
    

    def _load_edge_fuser(self, model, root_dir):
        if not hasattr(model, 'edge_fuser') or root_dir is None:
            return
        edge_path = os.path.join(root_dir, 'edge_fuser', 'pytorch_model.bin')
        if os.path.exists(edge_path):
            state = torch.load(edge_path, map_location='cpu')
            model.edge_fuser.load_state_dict(state, strict=True)
            log(f'Loading semantic-transition bridge from {edge_path}...')
        else:
            log(f'WARNING: semantic-transition bridge checkpoint not found: {edge_path}')
    ...
    def load(self, model, model_args={}):
        if not ('lora' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json'))): # loading model for non-lora/non-qlora pretraining
            ...
            self._load_edge_fuser(model, self.training_arguments.pretrained_model_path)
        else:
